...
public class UserListControllerTest {
    ...
    private void thenListResultShouldContainDtos(ResultActions result, List<UserDto> dtos)
            throws Exception {
        result.andExpect(MockMvcResultMatchers.jsonPath("$.data").isArray())
                .andExpect(MockMvcResultMatchers.jsonPath("$.data.length()").value(dtos.size()));

        for (int i = 0; i < dtos.size(); i++) {
            UserDto dto = dtos.get(i);
            result.andExpect(
                            MockMvcResultMatchers.jsonPath("$.data[" + i + "].id")
                                    .value(dto.getId()))
                    .andExpect(
                            MockMvcResultMatchers.jsonPath("$.data[" + i + "].username")
                                    .value(dto.getUsername()))
                    .andExpect(
                            MockMvcResultMatchers.jsonPath("$.data[" + i + "].email")
                                    .value(dto.getEmail()));
        }
    }
}
